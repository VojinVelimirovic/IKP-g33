...
#include <stdio.h>
#include "conio.h"
#include "stress_test.h"  // Include the stress test header
...
#define STRESS_TEST_THREADS_NUM 10
...
int main()
{
    ...
    while (strcmp(buffer, "x") != 0)
    {
        ...
        int command_code;
        printf("Choose a command:\n1. Allocate memory\n2. Free memory\n3. Run stress test\n4. Run stress test with sleep\nx. Quit\n");
        gets_s(buffer, BUFFER_SIZE);

        if (!strcmp(buffer, "3")) {
            run_stress_test(connectSocket, STRESS_TEST_THREADS_NUM, 0);
            continue;
        }

        if (!strcmp(buffer, "4")) {
            run_stress_test(connectSocket, STRESS_TEST_THREADS_NUM, 1); // Sleep enabled
            continue;
        }
    ...
    // Check if connection is successfully shut down.
